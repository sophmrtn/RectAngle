...
for ii in range(6):
    ...
    with torch.no_grad():
        ...
        for jj, (images_test, labels_test) in enumerate(test_DL):
            ...
            seg_predictions = torch.zeros_like(seg_model(norm_images_test))
            for seg_model in seg_models:
                seg_predictions += seg_model(norm_images_test)
            seg_predictions = seg_predictions / 3
            seg_predictions[seg_predictions > 0.5] = 1
            seg_predictions[seg_predictions < 1] = 0
